...
!function (a) {
    ...
    function b(c, d) {
        var e = a.noop, f = this, g = {
            ...
            autoSelectFirst: false,  //是否自动填充查询列表的第一项，默认值：false
            appendTo: document.body,  //查询列表容器被添加到那个元素中，默认值：document.body
            serviceUrl: null,  //服务器端的URL或者是返回 Url 字符串的回调函数
            lookup: null,  //查询的数据列表。字符串数组或者对象字面量（格式 { value: 'string', data: any }）
            onSelect: null,  //function (suggestion) {} ，用户选择查询结果后的毁掉函数
            width: "auto",  //提示容器的宽度，默认值：auto
            minChars: 1,  //触发提示的最小单词数，默认值：1
            maxHeight: 300,   //提示列表容器的最大高度，默认值：300
            deferRequestBy: 0,  //延迟Ajax请求的毫秒数，默认值：0
            params: {},   //参数传递的请求，可选
            formatResult: b.formatResult,  //function (suggestion, currentValue) {}
            formatGroup: b.formatGroup,
            delimiter: null,  //字符串或正则表达式，分割输入值并将最后一个作为查询词，一般逗号分割
            zIndex: 9999,  //提示容器的z-index值，默认值：9999
            type: "GET",  //获取提示的 Ajax 请求方式，默认值：get
            noCache: false,  //是否缓存提示结果，默认值：false
            onSearchStart: e,  //function (query) {}
            onSearchComplete: e,  //function (query, suggestions) {}
            onSearchError: e,  //function (query, jqXHR, textStatus, errorThrown) {}
            preserveInput: false,
            containerClass: "autocomplete-suggestions",
            tabDisabled: false,
            dataType: "text",  //服务器返回的数据格式
            currentRequest: null,
            triggerSelectOnValidInput: true,  //如果匹配查询，只要聚焦 input 就触发 onSelect 函数，默认值：true
            preventBadQueries: true,
            lookupFilter: function (a, b, c) {   //function (suggestion, query, queryLowerCase) {} 本地数据查询的过滤函数
            ...
            showNoSuggestionNotice: true,  //如果查询结果为空是否有提示语
            noSuggestionNotice: "搜索无结果",  //提示语
            orientation: "bottom",  //提示容器的垂直位置，默认值：'bottom'，可选值'top','auto'
    ...
    b.utils = c, a.Autocomplete = b, b.formatResult = function (a, b) {
        ...
        }, onSelect: function (b) {  //function (suggestion) {} ，用户选择查询结果后的毁掉函数
